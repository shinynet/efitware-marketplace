...
- **Claude app users (web and Desktop):** the plugin bundles this server, but the connection is made from the plugin page, not from install. Path: Customize → Plugins → Yours → eFitware → **Connectors** tab → Connect → Add → Connect → sign in to eFitware. When this skill is loaded but no eFitware tools are available in the conversation, say exactly that — the skill is loaded and its tools are not available in this conversation — then offer that path to check, add or connect the server. Missing tools alone do not establish that the server is disconnected or signed out: a connected account with the connector excluded from the conversation, disabled tools, failed discovery or a stale session looks the same, so state a disconnection only when host status or an authentication response establishes it. Offer Settings → Connectors → Add custom connector with the endpoint above only as the alternative. Never say the plugin is missing, and never claim installing or reinstalling the plugin can grant access. If the user needs to add, update or remove the marketplace itself, send them to claude.ai (Customize → Plugins → Add → Add marketplace, or Add → Manage marketplaces for updates, automatic sync and removal): the Desktop app's Add marketplace dialog does not register a marketplace and has no management menu.
...
A question about an existing workout, such as “Do I have a workout scheduled today?”, is a display request even when the user does not ask for a card. Resolve the user's local day, call `get_workout` with that `date`, then open the saved workout with `open_workout`. A scheduled occurrence that has no saved workout yet has no workoutId: call `open_calendar` for that day instead, and never create a workout just to show it. When several saved workouts match, name them and open the one the user means; when nothing is planned, say so without opening a card. Looking up or opening a workout never starts, reschedules or changes it.
