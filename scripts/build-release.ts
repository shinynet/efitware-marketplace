...
const paths = ['workout.html', 'efitware-codex.zip', 'efitware-claude.zip', 'efitware-coach.zip', 'contract/workout-view.schema.json', 'contract/compat-fixtures.json']
...
const manifest = { version: plugin.version, commit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), serverContract, skillSha256: sha256(skill), cardSha256: files['workout.html']!.sha256, themeSha256: theme.digest('hex'), files }
